...
void Robot_Call_Kinematics_Inverse(robot_object_t* p_robot, point_t *p_point_target){
    if(_has_end_effector_changed) { 
        ...
        if(Calculate_Kinematics_Inverse(p_robot, p_point_target, &(p_robot->_theta_current))){
            p_robot->_has_end_effector_changed = false; // Đặt cờ này thành false sau khi đã tính toán thành công, để tránh tính toán lại nếu điểm mục tiêu không thay đổi.
...
void Robot_Call_Kinematics_Forward(robot_object_t* p_robot, theta_t *p_theta_target){
    if(_has_theta_changed) {
        if(Calculate_Kinematics_Forward(p_robot, p_theta_target, &(p_robot->_end_effector_current))){
            p_robot->_has_theta_changed = false; // Đặt cờ này thành false sau khi đã tính toán thành công, để tránh tính toán lại nếu góc theta không thay đổi.
